@foreach ($data as $item)
    <tr class="{{ $loop->even ? 'bg-neutral-100' : 'bg-neutral-200' }} h-10 text-neutral-600 divide-x-2 divide-white">
        ...
        <td class="px-2 sm:px-4 py-1 min-h-10 font-semibold max-w-full break-all">
            @if ($item->article_type === \App\Models\Article::TYPE_ARTICLE_SPINTAX)
        ...
        <td class="px-1 sm:px-2">
            <div class="flex gap-1 sm:gap-2 justify-center">
                <button @click="detail({{ $item->id }})"
                    ...
                    <svg id="Layer_1" style="enable-background:new 0 0 512 512;" version="1.1" viewBox="0 0 512 512" class="w-full h-full" xml:space="preserve" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="M98.9,184.7l1.8,2.1l136,156.5c4.6,5.3,11.5,8.6,19.2,8.6c7.7,0,14.6-3.4,19.2-8.6L411,187.1l2.3-2.6c1.7-2.5,2.7-5.5,2.7-8.7c0-8.7-7.4-15.8-16.6-15.8v0H112.6v0c-9.2,0-16.6,7.1-16.6,15.8C96,179.1,97.1,182.2,98.9,184.7z"/></svg>
                </button>
                @if ($item->article_type === \App\Models\Article::TYPE_ARTICLE_SPINTAX)
                    <a href="{{ route('article-page.spin', ['id' => $item->id]) }}" target="__blank">
                        <button class="w-5 h-5 hover:text-blue-500 duration-300 relative group">
                            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M20 10H4c-1.1 0-2 .9-2 2s.9 2 2 2h16c1.1 0 2-.9 2-2s-.9-2-2-2zM4 8h12c1.1 0 2-.9 2-2s-.9-2-2-2H4c-1.1 0-2 .9-2 2s.9 2 2 2zM16 16H4c-1.1 0-2 .9-2 2s.9 2 2 2h12c1.1 0 2-.9 2-2s-.9-2-2-2z" fill="currentColor"></path></svg>
                            <div class="absolute gap-0.5 hidden group-hover:flex left-1/2 bottom-full bg-black text-white text-xs px-1 py-1 rounded-md">
                                ...
                                /
                                <span class="text-red-500">{{ $item->articleshow->count() }}</span>
                            </div>
                        </button>
                    </a>
                    <div class="relative" x-data="{ open: false, generatemodal:false }">
                        ...
                        <div x-show="open" @click.outside="open = false" class="absolute z-10 mt-1 right-0 bottom-0 bg-white border rounded shadow-lg w-32">
                            <a href="{{ route('article-page.show', ['article' => $item->id]) }}" class="block px-4 py-2 text-sm hover:bg-gray-100">Edit</a>
                            <a href="{{ route('article-page.shuffle.image', ['id' => $item->id]) }}" class="block px-4 py-2 text-sm hover:bg-gray-100">Shuffle Image</a>
                        ...
                        <x-admin.component.generatemodal :id="$item->id" :title="$item->judul" :route="route('article-page.generate', ['id' => $item->id])"/>
                    </div>
                    <div class="relative" x-data="{ open: false, deletemodal: false, deletegeneratemodal: false }">
                        ...
                        <div x-show="open" @click.outside="open = false" class="absolute z-30 mt-1 right-0 bottom-0 bg-white border rounded shadow-lg w-52">
                            ...
                            <div x-show="deletegeneratemodal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40">
                                <div class="w-full max-w-[720px] bg-white pb-6 rounded-md flex flex-col gap-4 relative overflow-hidden border-2 border-byolink-1">
                                    <button @click="deletegeneratemodal = false" class="absolute top-6 right-6 w-6 h-6 text-white hover:text-red-500 duration-300">
                                        <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M437.5 386.6 306.9 256l130.6-130.6c14.1-14.1 14.1-36.8 0-50.9-14.1-14.1-36.8-14.1-50.9 0L256 205.1 125.4 74.5c-14.1-14.1-36.8-14.1-50.9 0-14.1 14.1-14.1 36.8 0 50.9L205.1 256 74.5 386.6c-14.1 14.1-14.1 36.8 0 50.9 14.1 14.1 36.8 14.1 50.9 0L256 306.9l130.6 130.6c14.1 14.1 36.8 14.1 50.9 0 14-14.1 14-36.9 0-50.9z" fill="currentColor"></path></svg>
                                    </button>
                                    ...
                                    <form action="{{ route('article-page.generate.destroy', ['id' => $item->id]) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <div class="px-6 pb-4">
                                        ...
                                        <div class="flex justify-end space-x-4 px-6">
                                            <button @click="deletegeneratemodal = false" type="button" class="px-4 py-2 text-sm sm:text-base bg-neutral-600 duration-300 hover:bg-byolink-1 text-white rounded-md">Cancel</button>
                                            <button type="submit" class="px-4 py-2 bg-red-500 duration-300 hover:bg-red-900 text-white rounded">Hapus</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                            <x-admin.component.deletemodal :title="$item->judul" :route="route('article-page.destroy', ['article' => $item->id])" />
                        </div>
                    </div>
                @else
                    <a href="{{ route('article-page.show', ['article' => $item->id]) }}"
