<x-admin.article.form
    head="Edit Article Spintax"
    title="Admin - Edit Article Spintax"
    :spintax="route('article-page.spin', ['id' => $article->id])"
    :form="route('article-page.update', ['article' => $article->id])"
    indexRoute="article-page.index"
    indexLabel="Article">
    @method('PUT')
    <input type="hidden" name="type" value="spintax">
    ...
    <x-slot:additional>
        <div x-data="bannerComponent({{ $article->articlebanner }}, {{ $article->id }})" class="flex flex-col gap-2">
            <label class="text-sm sm:text-base font-semibold" for="article_page_edit_banner">Banner (Max 6)</label>
            <input type="file" class="hidden" id="article_page_edit_banner" name="image_banner[]" multiple accept="image/*" @change="addImages($event)">
            <div class="w-full grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-4">
                ...
                <template x-if="images.length < 6">
                    <div class="w-full aspect-square border bg-neutral-100 border-byolink-1 rounded-md relative border-dashed overflow-hidden">
                        <label for="article_page_edit_banner" class="w-full text-byolink-1 h-full absolute top-0 left-0 flex justify-center items-center p-[35%] hover:bg-byolink-3 hover:text-white/50 duration-300 cursor-pointer">
        ...
        <div x-data="galleryComponent({{ $article->articlegallery }}, {{ $article->id }})" class="flex flex-col gap-2">
            <label class="text-sm sm:text-base font-semibold" for="article_page_edit_gallery">Galeri (Max 12)</label>
            <input type="file" class="hidden" id="article_page_edit_gallery" name="image_gallery[]" multiple accept="image/*" @change="addImages($event)">
            <div class="w-full grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-4">
                <template x-for="(image, index) in images" :key="index">
                    <div class="w-full aspect-square flex items-center rounded-md relative overflow-hidden shadow-md shadow-black/20">
                        <img :src="image.url" class="w-full h-full object-cover" alt="Gallery Image Preview">
                ...
                <template x-if="images.length < 12">
                    <div class="w-full aspect-square border bg-neutral-100 border-byolink-1 rounded-md relative border-dashed overflow-hidden">
                        <label for="article_page_edit_gallery" class="w-full text-byolink-1 h-full absolute top-0 left-0 flex justify-center items-center p-[35%] hover:bg-byolink-3 hover:text-white/50 duration-300 cursor-pointer">
                            <svg viewBox="0 0 24 24" class="w-full h-full" xmlns="http://www.w3.org/2000/svg"><path d="m9 13 3-4 3 4.5V12h4V5c0-1.103-.897-2-2-2H4c-1.103 0-2 .897-2 2v12c0 1.103.897 2 2 2h8v-4H5l3-4 1 2z" fill="currentColor"></path><path d="M19 14h-2v3h-3v2h3v3h2v-3h3v-2h-3z" fill="currentColor"></path></svg>
                        </label>
                    </div>
                </template>
                        ...
                        files.forEach(file => {
                            ...
                            axios.post('/admin/article-banner', formData)
                                .then(response => {
                                    ...
                                    reader.onload = (e) => {
                                        this.images.push({ id: response.data.id, url: e.target.result });
                                    };
                                    reader.readAsDataURL(file);
                                })
                                .catch(() => this.errorMessage = 'Error uploading image. Please try again.');
                        });
                    },
                    deleteImage(index) {
                        ...
                        axios.delete(`/admin/article-banner/${image.id}`)
                            ...
                            .catch(() => this.errorMessage = 'Error deleting image. Please try again.');
                    }
                };
            }
