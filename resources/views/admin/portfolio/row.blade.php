        ...
        <td class="px-1 sm:px-2">
            <div class="flex gap-1 sm:gap-2 items-center justify-center">
                    ...
                    <div x-show="editmodal"
                        ...
                        <div
                            ...
                            <div class=" pt-6 pb-3 bg-byolink-1 text-white">
                                <h2 class=" px-6 text-2xl font-bold">Edit Portfolio</h2>
                            </div>
                            <form action="{{ route('portfolio.update', ['portfolio' => $item->id]) }}" method="POST" enctype="multipart/form-data"
                                        ...
                                        <div class=" w-full flex flex-col max-w-full gap-2 text-sm sm:text-base font-medium">
                                            ...
                                            <div class="w-full h-52 sm:h-60 flex items-center justify-center">
                                                <div class=" aspect-[3/2] max-h-full max-w-full rounded-md overflow-hidden shadow-md shadow-black/20 ">
                                                    <x-admin.component.imageinput title="Nama/Tipe" placeholder="Masukkan nama/tipe web..." value="{{ $item->image ? asset('storage/images/portfolio/'. $item->image) : null}}" :id="'-'.$item->id" name="image" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div x-data="{ deletemodal: false }" class=" w-5 h-5">
                    ...
                    <x-admin.component.deletemodal :title="$item->title" :route="route('portfolio.destroy', ['portfolio' => $item->id])" />
                </div>
            </div>
        </td>
    </tr>
