...
<div class="" x-data="{open : false}">
        ...
        <div class=" w-full max-w-[1080px] mx-auto flex items-center gap-10 justify-between">
            ...
            <div class=" hidden md:block flex-grow">
                <form action="{{route('allarticle')}}" method="get">
                    <div class=" flex items-center justify-between h-10 bg-white">
                        <input type="text" name="search" value="{{ request('search') }}" class=" min-w-0 sm:flex-grow h-10 text-sm px-4 sm:px-6 border-r-0 rounded-l-full focus:border-byolink-1 focus:ring-0" placeholder="Cari Tipe Desain....">
            ...
            <div class=" hidden md:flex flex-row gap-6 items-center text-neutral-500">
                ...
                <div x-data="{ article : false }" class=" relative">
                    <button @click="article = !article" class="{{ request()->routeIs('allarticle', 'pageallarticle', 'author', 'pageauthor', 'category', 'pagecategory', 'tag', 'pagetag') ? 'text-byolink-1' : 'hover:text-black hover:-translate-y-1'}} text-lg font-black py-2 duration-300" aria-label="Tipe Desain">Tipe Desain</button>
    ...
    <div :class="{' top-[70px] sm:top-20': open, '-translate-y-full top-0': !open}" class=" fixed flex md:hidden flex-col bg-white w-full left-0 justify-center gap-4 font-semibold text-neutral-600 pt-2 px-4 pb-4 duration-300 z-40">
        ...
        <form action="{{route('allarticle')}}" method="get">
            <div class=" flex items-center justify-between h-10 bg-white">
                <input type="text" name="search" value="{{ request('search') }}" class="flex-grow h-10 text-sm px-4 sm:px-6 border-r-0 rounded-l-full focus:border-byolink-1 focus:ring-0" placeholder="Cari Tipe Desain....">
