<div class=" w-full flex flex-col rounded-md overflow-hidden border bg-white shadow-md shadow-black/20">
    ...
    <div class=" pt-2 pb-2 sm:pb-4 px-2 sm:px-4 text-xs sm:text-sm flex flex-grow flex-col gap-1 justify-between">
        ...
        <a href="{{ route('business', ['slug' => $item->slug]) }}" aria-label="{{$item->judul}}" class=" pt-1">
            <button class=" w-full py-1 sm:py-1.5 gap-1 flex justify-center rounded-full text-xs sm:text-sm font-semibold bg-byolink-2 text-white hover:bg-byolink-1 duration-300">
